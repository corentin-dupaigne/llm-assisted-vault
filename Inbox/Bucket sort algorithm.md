...
2. **Scatter** — walk the input once, drop each element into its bucket (`index = f(value)`, no comparison).
3. **Sort** each bucket individually (insertion sort, or built-in sort).
4. **Gather** — concatenate buckets in order. No merging: bucket 0 < bucket 1 < ... so a simple `append`.
