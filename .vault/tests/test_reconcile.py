...
so editing a filed note's frontmatter or title by hand leaves `vault.index.json`
stale forever. `reconcile_index` re-derives the index from the notes actually on
disk; these tests prove a manual edit, a deletion, a rename and a hand-added note
...
def _note(*, title, domain, tags, para, project="null", date="2026-06-04",
          body="\nReference content.\n"):
    tags_line = "[" + ", ".join(tags) + "]"
    return (
        f"---\ndomain: {domain}\ntags: {tags_line}\ndate: {date}\n"
...
def test_edited_metadata_and_title_reach_the_index(vault):
    """Hand-editing a filed note's frontmatter + H1 refreshes its index entry."""
    _file(vault, "Resources/photosynthesis.md",
          _note(title="Photosynthesis Reworked", domain="botany",
                tags=["plants", "energy"], para="Resources"))
    ...
    entry = index["notes"][0]
    assert entry["title"] == "Photosynthesis Reworked"
...
def test_deleted_note_is_pruned_with_its_orphaned_metadata(vault):
    ...
    _file(vault, "Resources/kept.md",
          _note(title="Kept", domain="general", tags=["shared"], para="Resources"))
    vault.seed_index(
        notes=[
...
def test_hand_added_note_gets_indexed_with_location_derived_placement(vault):
    ...
    from its location, not its (here absent) placement frontmatter."""
    _file(vault, "Projects/cka/etcd-backup.md",
          _note(title="Etcd Backup", domain="kubernetes", tags=["etcd"],
                para="Projects", project="cka"))
...
def test_in_sync_vault_is_a_noop(vault):
    ...
    _file(vault, "Areas/health.md",
          _note(title="Health", domain="wellbeing", tags=["habits"], para="Areas"))
    vault.seed_index(notes=[{
